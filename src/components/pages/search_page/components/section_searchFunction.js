...
export function searchFunction([renderResults, setRenderResults]) {
    const form = document.querySelector('.search__container__content--main--form--FORM'),
          formBtnSubmit = document.querySelector('.search__container__content--main--form__container--checkbox--containerButton__container__content--btnContainer__btn');


    const inputForm = document.querySelector('.search__container__content--main--form__container--input'),
          inputInn = document.querySelector('.search__container__content--main--form__container--input__container--inn--input'),
          selectTone = document.querySelector('.search__container__content--main--form__container--input__container--tone--select'),
          inputQuantityInIssue = document.querySelector('.search__container__content--main--form__container--input__container--quantityInIssue--input'),
          inputDateRange1 = document.querySelector('.search__container__content--main--form__container--input__container--dateRange--input1'),
          inputDateRange2 = document.querySelector('.search__container__content--main--form__container--input__container--dateRange--input2'),
          markerErrInn = document.querySelector('.search__container__content--main--form__container--input__container--inn--label__symbol'),
          markerErrDateRange = document.querySelector('.search__container__content--main--form__container--input__container--dateRange--label__symbol'),
          messageErrInn = document.querySelector('.search__container__content--main--form__container--input__container--inn--input--err'),
          messageErrDateRange = document.querySelector('.search__container__content--main--form__container--input__container--dateRange--input--err');

    // Чекбоксы
    const allcheckbox = document.querySelectorAll('.search__container__content--main--form__container--checkbox--containerCheckbox__container--input');
    ...
    function handleInput() {      
        ...
        if (inpDateRange1 === "" || inpDateRange2 === "") {
            validValueDateRange = false;
        } else if (inpDateRange1Y > inpDateRange2Y || inpDateRange1M > inpDateRange2M) {
            validValueDateRange = false;
        } else if (inpDateRange1Y > year || 
                (inpDateRange1Y === year && inpDateRange1M > month) ||
                (inpDateRange1Y === year && inpDateRange1M === month && inpDateRange1D > day) ||
                inpDateRange2Y > year || 
                (inpDateRange2Y === year && inpDateRange2M > month) ||
                (inpDateRange2Y === year && inpDateRange2M === month && inpDateRange2D > day)) {
            validValueDateRange = false;
    ...
    function renderBtnSubmit() {
        if (inputQuantityInIssue.value.length > 0 && validValueInn === true && inputInn.value.length === 10 && validValueDateRange === true) {
    ...
    allcheckbox.forEach(input => {
        input.addEventListener('click', event => {
            const nameCheckbox = document.querySelector('.' + input.getAttribute('data-info'));
